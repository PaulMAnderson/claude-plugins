---
title: Astrolabe · CONTEXT
version: 2
id: f4222b395215d9b083d67639c93a8b07
createdAt: 2026-09-17T11:58:27.997Z
updatedAt: 2026-09-17T11:58:27.997Z
---
> Imported from `.astrolabe/CONTEXT.md` on 2026-09-17. The original file remains in place.
...
## Current checkout after merge and rename
The user confirmed renaming the repository folder. The checkout is now `/home/paul/Claude/astrolabe` on local `main` at `93a4a50`; the previous `claude-plugins` path and feature branch references above describe historical execution state. The feature branch was fast-forward merged into `main`. All 13 project and 13 user Astrolabe skill links resolve after updating the user links. Post-move checks: 5 Hypercube tests, 23 Codex tests, journaling, and both run-ledger checks passed. Local main is ahead of origin/main; no push was requested.

## B2 legacy migration (2026-09-16)
User started B2 on `/home/paul/Claude/astrolabe`; branch `feature/legacy-rpi-migration`, execution base `bf4fd04`. Added `scripts/migrate-rpi-project.py` and engine to preview and copy `.rpi/` state, preserve source, archive raw SESSION and unknown files, import session text into HISTORY, copy guidance and root plans, and publish a schema-1 `.astrolabe/` after staging. Existing targets conflict; completed migration reruns are no-ops. Design and plan are in `.astrolabe/docs/`; ledger `.astrolabe/runs/legacy-rpi-migration/state.json` passes 11/11 gates. Final checks: 10 repository tests, 23 Codex tests, journaling, CLI help, and diff whitespace passed. B2 was completed in HISTORY and removed from PLANNED. No external project was migrated. Branch has not been merged or pushed.
