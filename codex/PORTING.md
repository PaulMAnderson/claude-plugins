...
The original Claude and Gemini trees are retained. The Codex implementation is a maintained skills bundle in `codex/skills`, discoverable through project `.agents/skills` links or the installer. All 39 Claude skills and 38 Gemini skills are represented; the Gemini tree lacks Claude's `receiving-code-review`, which is included in `rpi-review`.

`source-map.json` records every source skill plus command, agent, hook, and workflow script (148 files), its SHA-256, and its target skill. This provides a complete inventory and flags upstream drift. It is a coverage map, not a mechanical string-replacement generator.
