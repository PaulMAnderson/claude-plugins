---
name: "astrolabe-implement"
description: "Execute or resume an Astrolabe implementation plan with task reports, durable progress, per-phase review and fixes, final acceptance coverage, and evidence-based validation."
---
...
4. Execute tasks in dependency order, with status and a durable report for each. Use [astrolabe-house-style](../astrolabe-house-style/SKILL.md) when its conventions apply. Test behavioral changes red → green → refactor; use direct operational checks for infrastructure. Address task concerns before claiming completion. Show the task's concise outcome and verification, then continue.
5. Review the entire phase after its tasks using [astrolabe-review](../astrolabe-review/SKILL.md). Record every finding verbatim, fix valid Critical/Important/Minor issues, rerun affected checks, and explicitly re-review them. A task's success report alone cannot complete the phase review gate. Continue only when no unresolved findings remain.
6. After all phases, inspect changed contracts/commands/structure and update relevant project context as needed with [astrolabe-extend-codex](../astrolabe-extend-codex/SKILL.md). Run full-change review, acceptance/test coverage analysis, and final verification in that order using `astrolabe-review`. If later fixes change code, re-review the affected and cross-phase behavior and refresh invalidated checks.
