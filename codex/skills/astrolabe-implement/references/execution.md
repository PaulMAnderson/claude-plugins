...
Write reports under `.astrolabe/runs/<slug>/reports/phase-N-task-M.md`:
...
The state helper is at `../../astrolabe-workflow/scripts/astrolabe_state.py` relative to this reference directory. Resolve it to an absolute path before working in the target repository. Always pass `--root` and `--run` explicitly. Example (replace paths and slug with actual values):

```sh
python3 /path/to/astrolabe_state.py --root /path/to/project --run feature status
python3 /path/to/astrolabe_state.py --root /path/to/project --run feature gate phase:1:read in_progress
python3 /path/to/astrolabe_state.py --root /path/to/project --run feature gate phase:1:read completed --evidence /path/to/read-report.md
```
