...
1. Announce the relevant skill and current objective. Read applicable `AGENTS.md` instructions, existing `.astrolabe/CONTEXT.md` and `.astrolabe/PROJECT.md`, and the current plan. Verify the repository/worktree, branch, actual files, and working-tree changes before trusting saved state.
...
4. Scale artifact size to the task. A formal plan retains all gates even when compact. One-off analysis and small helpers use `astrolabe-quick`; do not force a multi-phase project onto them.

## Progress is durable

Use the bundled `scripts/astrolabe_state.py` (relative to this skill directory) to create and maintain `.astrolabe/runs/<slug>/state.json`. Run its `--help` and `init --help` for exact arguments. This is the authoritative machine-readable ledger; `.astrolabe/CONTEXT.md` is the human resume summary; `PROJECT.md` remains the stable description. A native planning tool, when exposed, may mirror it but does not replace it.
...
Before handoff/compaction, merge active state into `.astrolabe/CONTEXT.md` and append the outcome to `.astrolabe/HISTORY.md`. Preserve exact constraints, unresolved findings, evidence, absolute plan/worktree paths, recorded bases, and next gate. Re-read and reconcile on resume. Do not require `/clear` or pretend context is automatically injected by an uninstalled hook.
