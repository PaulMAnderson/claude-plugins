#!/usr/bin/env python3
"""Durable, single-controller RPI gate and issue ledger (Python standard library)."""
...
def run(args):
    ...
    directory = (root / ".rpi" / "runs" / args.run).resolve()
...
def main(argv=None):
    try:
        ...
        print(f"rpi-state: {error}", file=sys.stderr)
        return 2
