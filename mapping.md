...
This skill should be used when the user asks to "convert Claude plugin to Gemini", "map Claude components to Gemini", "translate plugin format", "convert commands to TOML", "convert skills to GEMINI.md", or needs to understand how Claude Code plugin components map to Gemini CLI extension equivalents.

| Field      | Value                                           |
| ---------- | ----------------------------------------------- |
| Identifier | `orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping`                                |
| Version    | 0.1.0                                     |
| Author     | orbruno                                 |
| Category   | ai-llms                                    |
| Installs   |                                 |
...
**GitHub:** [orbruno/claude-to-gemini-ccplugin](https://github.com/orbruno/claude-to-gemini-ccplugin) — ⭐  | Forks: 
...
This Skill provides a practical mapping guide and conversion workflow for migrating Claude Code plugins to Gemini CLI extensions. It details one-to-one correspondences for core components (manifest fields, endpoints, handlers, schemas, scopes, and auth), explains how to translate command definitions into TOML, and shows how to generate a GEMINI.md skill document that preserves behavior and documentation. Use cases include porting existing Claude plugins to Gemini, generating starter CLI extension scaffolds, converting command formats, and validating parity between implementations. Key features: component mapping rules, manifest and schema conversion patterns, example conversions, recommended testing and validation steps, and notes on permission/auth differences. The Skill reduces manual errors, speeds migration, and produces developer-ready Gemini extensions with consistent documentation and configuration.
...
```bash
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping
```
...
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --agent open-claw

# Claude Code (installs to ./.claude/skills/)
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --agent claude-code

# Codex (installs to ./.agents/skills/)
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --agent codex

# Cursor (installs to ./.cursor/skills/)
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --agent cursor
```

Install a specific version:

```bash
npx -y @lobehub/market-cli skills install orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --version 0.1.0
```
...
```bash
npx -y @lobehub/market-cli skills rate orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping --score <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills comment orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping -c "Your feedback here" --rating <1-5>
```
...
```bash
npx -y @lobehub/market-cli skills uncomment orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping
```
...
```bash
npx -y @lobehub/market-cli skills comments orbruno-claude-to-gemini-ccplugin-claude-gemini-mapping
```
