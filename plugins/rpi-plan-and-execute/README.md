...
|--------|------------------|--------------|
| **rpi-research-agents** | `codebase-investigator`, `internet-researcher` | Codebase verification, external research during design |
| **rpi-house-style** | `coding-effectively` and sub-skills | Code quality standards during implementation and review |
| **rpi-extending-claude** | `project-claude-librarian` | Updating CLAUDE.md files after implementation |
...
| Agent | Plugin | Role |
|-------|--------|------|
| **codebase-investigator** | rpi-research-agents | Verifies file paths, finds patterns, confirms assumptions |
| **internet-researcher** | rpi-research-agents | Finds current API docs, library patterns, best practices |
| **task-implementor-fast** | rpi-plan-and-execute | Implements tasks with TDD, runs verification, commits |
| **code-reviewer** | rpi-plan-and-execute | Enforces quality standards, blocks on issues |
| **task-bug-fixer** | rpi-plan-and-execute | Fixes issues identified by code reviewer |
| **test-analyst** | rpi-plan-and-execute | Validates test coverage against acceptance criteria, generates human test plans |
| **project-claude-librarian** | rpi-extending-claude | Updates CLAUDE.md files when contracts change |
