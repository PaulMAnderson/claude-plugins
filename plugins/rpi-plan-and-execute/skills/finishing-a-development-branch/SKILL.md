...
Before merging or creating a PR, invoke `rpi-extending-claude:project-claude-librarian` to update CLAUDE.md files if contracts or structure changed.
...
**If librarian subagent is not available:** skip this step, saying aloud that you're skipping it because the `rpi-extending-claude` plugin is not available.
