...
Follow [the shared project state entry and exit contract](../../references/project-state.md) before and after this skill. Use tier `design`. Initialize missing files, read `PROJECT.md` and `STATUS.md`, and consult `PLANNED.md` before work. Record the actual outcome in `HISTORY.md` and update `STATUS.md` on exit.
...
Options:
  - [list any design plans you find in .astrolabe/docs/design-plans/]
  - "Let me provide the path"
```

If `.astrolabe/docs/design-plans/` doesn't exist or is empty, ask the user to provide the path directly.
...
(conditional) TaskCreate: "Read project implementation guidance from [absolute path]"
  ...
  → (only if .astrolabe/implementation-plan-guidance.md exists)
...
1. Implementation plan directory name (`.astrolabe/docs/implementation-plans/YYYY-MM-DD-{slug}/`)
...
1. **REQUIRED SUB-SKILL:** Use astrolabe-plan-and-execute:using-git-worktrees
...
**Check if `.astrolabe/implementation-plan-guidance.md` exists:**

Use the Read tool to check if `.astrolabe/implementation-plan-guidance.md` exists in the session's working directory.

**If the file exists:**

1. Use TaskCreate to add: "Read project implementation guidance from [absolute path to .astrolabe/implementation-plan-guidance.md]"
...
**REQUIRED SUB-SKILL:** Use astrolabe-plan-and-execute:writing-implementation-plans
...
- Write implementation plan to `.astrolabe/docs/implementation-plans/`
...
cat /path/to/plugins/astrolabe-plan-and-execute/skills/starting-an-implementation-plan/SKILL.md
```
...
Wait for the skill to confirm `.astrolabe/CONTEXT.md` has been written before proceeding.
...
ls -d "${WORKING_ROOT}/.astrolabe/docs/implementation-plans/YYYY-MM-DD-feature-name"
```
...
/astrolabe-plan-and-execute:execute-implementation-plan /Users/ed/project/.worktrees/oauth2-feature/.astrolabe/docs/implementation-plans/2025-01-17-oauth2-feature/ /Users/ed/project/.worktrees/oauth2-feature/
...
**Note:** `.astrolabe/CONTEXT.md` has been written. The next session will automatically
...
```
Design Plan (in .astrolabe/docs/design-plans/)
...
Starting Implementation Plan (this skill)
  ...
  -> Planning [tracked task wrapping granular tasks]
    ...
    -> Write to .astrolabe/docs/implementation-plans/
