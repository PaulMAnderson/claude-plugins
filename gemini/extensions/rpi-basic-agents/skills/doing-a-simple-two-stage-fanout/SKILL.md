...
If the user's intent is not already clear, ask two questions using `ask_user`:
...
Determine how many segments, workers, and critics the analysis requires.
...
Use the `run_shell_command` tool to count characters: `wc -c file1 file2 ...`.

For more precise estimates, run the `compute_layout.py` script bundled with this skill:

```bash
python3 ${extensionPath}/skills/doing-a-simple-two-stage-fanout/compute_layout.py --corpus-chars 800000 --segments-per 3 --reviews-per 2
```
...
Enter plan mode using `enter_plan_mode`. Write a plan that includes:
...
Do not proceed until the user approves the plan. Exit plan mode with `exit_plan_mode`.

## Step 4: Launch Workers

Launch all Workers in parallel. Use `write_todos` to track progress.
