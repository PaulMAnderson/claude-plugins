...
Follow [the shared project state contract](../../references/project-state.md) before and after this skill with tier `design`. Initialize missing files, read PROJECT.md and STATUS.md, and consult Backlog and Roadmap before work. Use `ask_user` to surface a matching entry and wait for the choice. On exit, append HISTORY.md and update STATUS.md with the shared helper.
...
Use the read_file tool to check if `.astrolabe/implementation-plan-guidance.md` exists in the session's working directory.
...
cat /path/to/gemini/extensions/astrolabe-plan-and-execute/skills/starting-an-implementation-plan/SKILL.md
```
