# Writing Implementation Plans — Reference

Detailed templates, workflows, and processes. Read specific sections when SKILL.md directs you to them.
...
```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-plan-and-execute:code-reviewer</parameter>
...
<parameter name="prompt">
  ...
  DESIGN_PLAN: [path to design plan, e.g., .astrolabe/docs/design-plans/YYYY-MM-DD-feature.md]

  IMPLEMENTATION_GUIDANCE: [absolute path to .astrolabe/implementation-plan-guidance.md, or "None" if file does not exist]
...
1. **Create a task for EACH issue** (survives compaction):
   ```
   write_todos: "Finalization fix [Critical]: <VERBATIM issue description from reviewer>"
   write_todos: "Finalization fix [Important]: <VERBATIM issue description from reviewer>"
   write_todos: "Finalization fix [Minor]: <VERBATIM issue description from reviewer>"
   ...one task per issue...
   write_todos: "Finalization: Re-review after fixes"
   write_todos: set "Re-review" blocked by all fix tasks
   ```
...
```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-basic-agents:opus-general-purpose</parameter>
...
- **Interactive mode:** Present to user, ask_user for approval. This is the LAST interactive item.
...
1. **Create a task for EACH revision** (survives compaction):
   ```
   write_todos: "Test requirements fix: <VERBATIM revision request from user>"
   ...one task per revision...
   write_todos: "Test requirements: Re-present for approval"
   write_todos: set "Re-present" blocked by all fix tasks
   ```
...
**"Implementation plan complete and validated. Saved to [count] phase files + test-requirements.md in `.astrolabe/docs/implementation-plans/YYYY-MM-DD-<feature-name>/`. The first phase file is `<full-path>`. Test requirements are in `<full-path>/test-requirements.md`."**
