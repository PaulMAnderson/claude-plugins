...
Follow [the shared project state contract](../../references/project-state.md) with tier `design` before beginning execution and when ending the run. Initialize missing files, read PROJECT.md and STATUS.md, consult Backlog and Roadmap, update STATUS.md, and append the actual outcome to HISTORY.md. On an incomplete run use `--status paused`; the ledger remains the detailed execution record.
...
Options:
  - [list any plan directories you find in .astrolabe/docs/implementation-plans/]
  - "Let me provide the path"
```

If `.astrolabe/docs/implementation-plans/` doesn't exist or is empty, ask the user to provide the path directly.
...
After discovering phases, check if `.astrolabe/implementation-plan-guidance.md` exists in the project root:

```bash
# Check for implementation guidance (note the absolute path for later use)
ls [project-root]/.astrolabe/implementation-plan-guidance.md
```
...
```bash
cat "$(git rev-parse --show-toplevel)/.astrolabe/exec/progress.md"
```
...
```bash
scripts/task-brief [absolute path to phase file] N   # prints e.g. .astrolabe/exec/task-N-brief.md
```

```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-plan-and-execute:task-implementor-fast</parameter>
...
<parameter name="prompt">
  ...
  Apply all relevant skills, such as (if available) astrolabe-house-style:coding-effectively.
...
```bash
scripts/review-package [phase BASE SHA] HEAD   # prints e.g. .astrolabe/exec/review-abc1234..def5678.diff
```
...
- IMPLEMENTATION_GUIDANCE: absolute path to `.astrolabe/implementation-plan-guidance.md` (**only if it exists**—omit entirely if the file doesn't exist)
...
```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-plan-and-execute:task-bug-fixer</parameter>
...
After all phases complete, invoke the `astrolabe-extending-claude:project-claude-librarian` subagent (when available) to review changes and update CLAUDE.md files if needed.

```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-extending-claude:project-claude-librarian</parameter>
...
<parameter name="prompt">
  ...
  Follow the astrolabe-extending-claude:maintaining-project-context skill to:
...
**If librarian subagent is unavailable:** skip this entire step. Say aloud that you're skipping it because the `astrolabe-extending-claude` plugin is not available.
...
```
<invoke name="Task">
<parameter name="subagent_type">astrolabe-plan-and-execute:test-analyst</parameter>
...
# e.g., impl plan dir: .astrolabe/docs/implementation-plans/2025-01-24-oauth/
